...
describe MobileAPI::API do
  ...
  describe "POST /users" do
    ...
    it "returns 400 status if there is no parameter 'password'" do
      post "/mobile_api/v1/users", email: "jevan@hua.li", password: "12345678", name: "Jevan", phone: "18758161801" 
  ...
  describe "PUT /users/change_password" do
    it "changes the password of user" do
      put "/users/change_password", password: user.password, new_password: "12345678"
      response.status.should == 200
    end
  end
  ...
  describe "PUT /users" do
    it "changes user information" do
      put "/users", name: user.name, phone: "18758161801"
      response.status.should == 200
    end
  end

  describe "GET /users/huali_points" do
    it "returns huali points of the user" do
      get "/users/huali_points", id: user.id
      response.status.should == 200

...
STR
  ...
  def sales_report(*emails)
    ...
    OPTIONS
    Highcharts::Converter.run(amount_chart_options)
    @report_body << File.read(Highcharts::Converter.outfile_path)
    ...
    OPTIONS
    Highcharts::Converter.run(count_chart_options)
    @report_body << File.read(Highcharts::Converter.outfile_path)
